...
- Registry: Activate the `registry` page based on the backend resource indicated in the `url` parameter. The registry page provides a table for viewing all records within a lifecycle, as well as pages to add, delete, edit, and view these records individually using a form UI. This endpoint should target the [VisBackendAgent](https://github.com/cambridge-cares/TheWorldAvatar/tree/main/Agents/VisBackendAgent).
