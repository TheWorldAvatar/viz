...
export function getCameraPositions(cameraSettings: CameraSettings): string[] {
  return cameraSettings.positions.map(
    (position: CameraPosition) => position.name
  );
}
 ...
 */
export function getDefaultImageryOption(
  ...
  if (
    ...
    imageryOptions.default.toLowerCase() == "auto"
  ) {
 ...
 */
export function getCurrentImageryOption(
  imageryOptions: ImageryOptions
): ImageryOption {
 ...
 * @param {Map} map The current Mapbox map instance.
 */
export function togglePlacenames(
  imageryOptions: ImageryOptions,
  map: Map
): void {
  const reduxState = reduxStore.getState();
...
export function resetCamera(cameraSettings: CameraSettings, map: Map): void {
  ...
  if (items.length == 0) {
    ...
    const positionName = items.find(
      (position) => position.id === "reset"
    )?.selection;
