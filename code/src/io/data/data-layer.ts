...
export abstract class DataLayer {
  ...
  */
  ...
  // indicates whether this layer is a highlight layer (hidden in the tree)
  public isAHighlightLayer: boolean;

  // the property to match on the layer (e.g. ogc_fid) to filter the selected feature
  public highlightFeatureId: string;
  ...
  constructor(id: string, isGroupExpanded: boolean, source: LayerSource, definition: object) {
    ...
    if (this.definition.isAHighlightLayer) {
      this.isAHighlightLayer = this.definition.isAHighlightLayer as boolean;
      this.highlightFeatureId = this.definition.highlightFeatureId as string;
    }
