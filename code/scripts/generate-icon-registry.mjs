...
// Record where each name was found, so errors can point at a real file.
const usage = new Map();
 ...
 */
function recordIcons(node, file) {
  if (Array.isArray(node)) {
    for (const item of node) recordIcons(item, file);
    return;
  }
  if (node === null || typeof node !== "object") return;
  for (const [key, value] of Object.entries(node)) {
    if (key === "icon" && typeof value === "string") record(value, file);
    else recordIcons(value, file);
  }
}
...
for (const file of ICON_CONFIG_FILES) {
  ...
  try {
    recordIcons(JSON.parse(src), file);
  } catch {
...
writeFileSync(
  ...
  [
    ...
    "// Icons here come from `\"icon\"` values in public/ config; components import lucide directly.",
