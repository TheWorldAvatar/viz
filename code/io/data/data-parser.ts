...
export class DataParser {
  ...
  private recurse(current: JsonObject, parentGroup: DataGroup, stack: string, depth: number) {
    ...
    if (current["search"]) {
      if (typeof current["search"] === "string") {
        ...
        dataGroup.search = current["search"] as SearchConfig;
      }
    }
  ...
  */
  private parseDataLayers(layerArray: JsonArray, dataGroup: DataGroup) {
    // if highlight is specified, the layer is duplicated with the highlight properties
    // the highglight layer is not shown in the tree and will only be shown when a feature is selected on the map
    const layerArrayWithHighlightDuplicate = layerArray.flatMap(item =>
      item.highlight
        ? [
          { ...item, hasHighlight: true },
          {
            ...item,
            ...(item.highlight as object),
            id: item.id + '-highlight',
            name: item.name + '-highlight',
            isAHighlightLayer: true
          }
        ]
        : [item]
    );
    for (const element of layerArrayWithHighlightDuplicate) {
      ...
      if (element.isAHighlightLayer) {
        // ensures that highlight layer does not show up on first load
        element.layout = { ...element.layout as object, visibility: "none" };
      }
